...
def apply_mosaic(img, x, y, w, h, mosaic_ratio=0.11):
    ...
    for i in range(y, y + h, mosaic_rate):
        for j in range(x, x + w, mosaic_rate):
            s_x = x if j - mosaic_rate < x else j - mosaic_rate
            e_x = x + w if j + mosaic_rate > x + w else j + mosaic_rate
            s_y = y if i - mosaic_rate < y else i - mosaic_rate
            e_y = y + h if i + mosaic_rate > y + h else i + mosaic_rate
